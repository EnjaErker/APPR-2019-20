...
t3 <- ggplot(place, aes(x="", y=Število, fill=Višina)) + geom_bar(stat="identity", width=1) +
  ...
  labs(x = NULL, y = NULL, fill = NULL, title = "Povprečne mesečne neto plače glede na slovensko povprečje leta 2015") +
  theme_classic() + theme(axis.line = element_blank(),axis.text = element_blank(),axis.ticks = element_blank(),plot.title = element_text(hjust = 0.5, color = "#666666"))+
  geom_text(aes(label = paste(round(Število / sum(Število) * 100, 1), "%"), x = 1.3),position = position_stack(vjust = 0.5))
...
z1 <- tm_shape(merge(obcine, gostota_z, by.x="OB_UIME", by.y="Obcina")) + tm_polygons("vrednost", title="Gostota naseljenosti (preb/km^2)") +
  tm_layout(title="Gostota prebivalstva Slovenije leta 2015", legend.position=c(0.65,0.005), legend.height=0.7, title.size=1.5)

#place
z2 <- tm_shape(merge(obcine, placa_indeks_z, by.x="OB_UIME", by.y="Obcina")) + tm_polygons("vrednost", title="Vrednost indeksa") + 
  tm_layout(title="Povprečne mesečne neto plače leta 2015",legend.position=c(0.82,0.005), legend.height=0.7, title.size=1.5) 

#brezposelni
z3 <- tm_shape(merge(obcine, brezposelni_z, by.x="OB_UIME", by.y="Obcina")) + tm_polygons("vrednost", title="Odstotki") + 
  tm_layout(title="Stopnja registrirane brezposelnosti v letu 2015",legend.position=c(0.82,0.005), legend.height=0.7, title.size=1.5) 
...
z4 <- tm_shape(merge(obcine, stanovanja_z, by.x="OB_UIME", by.y="Obcina")) + tm_polygons("vrednost", title="Število stanovanj na 1000 preb.") + 
  tm_layout(title="Število stanovanj v letu 2015",legend.position=c(0.68,0.005), legend.height=0.7, title.size=1.5) 
...
m1 <- ggplot(t, aes(x = Place, y = t$delez)) + 
  geom_point() + 
  geom_smooth(method=lm, se=FALSE) + labs(x = "Plače glede na slovensko povprečje (indeks = 100)", y = "Delež stanovanj brez centralnega ogrevanja (v %)", fill = NULL, title = "Ponazoritev premice linearne regresije - vpliv plač na centralno ogrevanje")
...
m2 <- ggplot(t, aes(x = Brezposelni_ods, y = t$delez)) + 
  geom_point() + 
  geom_smooth(method=lm, se=FALSE) + labs(x = "Stopnja brezposelnosti (v %)", y = "Delež stanovanj brez centralnega ogrevanja (v %)", fill = NULL, title = "Ponazoritev premice linearne regresije - vpliv brezposelnosti na centralno ogrevanje")
