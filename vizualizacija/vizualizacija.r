...
gostota <- data.table(Gostota = c("Zelo redka", "Redka","Gosta","Zelo gosta"), Število = c(st_zelo_redkih, st_redkih, st_gostih, st_zelo_gostih))
...
kriticnost <- data.table(Kritičnost = c("Kritična","Ne kritična"), Število = c(st_kriticnih, st_nekriticnih))
...
place <- data.table(Višina = c("Nizka", "Srednja", "Visoka"), Število = c(prva, druga, tretja))
...
pomankljivi <- data.table(Stopnja = c("Zelo nizka", "Nizka", "Visoka", "Zelo visoka"), Število = c(st_zelo_nizkih, st_nizkih, st_visokih, st_zelo_visokih))
